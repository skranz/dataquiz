example.eurostat_sbs = function() {
  ...
  app = rankquizApp(rq.fun=make.eurostat.na.industry.rank.quiz, dat=d, year=2012, base.year=2002, sector.digits=3)
  viewApp(app)
...
make.eurostat.na.industry.rank.quiz = function(dat, var=NULL, top.n=10, choice.n=top.n,  decreasing = TRUE, year=NULL, vars=NULL, base.year=NULL, country="DE", mode=sample(c("value","change","perc_change"),1), sector.digits = NULL) {
  ...
  rq = make.rankquiz(d,ts.dat = ts.dat,key="sector",value="value", question=paste0("Eurostat National Accounts: Find sectors with ", if (decreasing) "highest" else "lowest"," '", var.label,"' (Unit=",unit,")."), top.n=top.n, choice.n=choice.n, decreasing=decreasing)
